...
type NonNullWeatherData = WeatherData extends null ? never : WeatherData;

const WeatherMap: React.FC = () => {
  const [weatherDataList, setWeatherDataList] = useState<NonNullWeatherData[]>([]);
  ...
  useEffect(() => {
    const fetchDataForRegions = async () => {
      ...
      setWeatherDataList(weatherDataArray.filter((data): data is NonNullWeatherData => data !== null) as NonNullWeatherData[]);
    };
  ...
  <>
    ...
    <div style={{ textAlign: "center", marginTop: "20px" }}>
      {selectedMarker && (
        <>
          <p style={{ color: "black" }}>Temperature: {selectedMarker.main.temp}°C</p>
